...
function advanceBlock() {
  return promisify(
    (web3!.currentProvider as any).send.bind(web3.currentProvider)
  )({
    jsonrpc: "2.0",
    method: "evm_mine",
    id: new Date().getTime(),
...
async function advanceBlockTo(target: number | BN) {
  if (!BN.isBN(target)) {
...
async function increase(duration: number | BN) {
  if (!BN.isBN(duration)) {
 ...
 */
async function increaseTo(target: number | BN) {
  if (!BN.isBN(target)) {
    target = new BN(target);
  }
